# Zipper Unity 框架 — 技术路线规划（v0.4 待审批）

> 状态：**v0.4 草稿，待审批**
> 定位：本文件是框架开发的总体技术路线，回答「做什么、怎么做、按什么顺序做」；**不含代码实现**。
> 依据：协调者规范（先文档、后编码；无批准、不实施）；`docs/standards/agent-role.md`（AI 只做设计，代码由使用者实现）。
> 变更记录：v0.2 融入调研结论；v0.3 定为 **.unitypackage 右键导出、拆箱即用**（放弃 UPM 包形态）；**v0.4（2026-09-06）** 同步近期现实与决策——现状盘点更新、资源管理器键体系改为 address/label、Core 定为"零框架依赖"例外、新增事件总线决策、新增 Core 基础设施一节、里程碑状态刷新；**分发相关（UPM 依赖与"随包自包含"的冲突）按使用者决定暂缓，待其另行决策**。
...
| 设计文档 | 资源管理器设计稿 v0.3、Core 设计稿 v0.3 | 供实现参照（只含思路级伪代码） |
...
| UI 体系 / 架构 | uGUI + TMP；**MVVM**，绑定层用 R3 | R3 尚未引入 |
| 依赖注入 | VContainer | 模块经容器装配 |
| 异步 | UniTask（对外接口统一） | 已切 UPM |
| 对象池（普通） | 自研 `ZObjectPool<T>` 演进 | 已有骨架 |
| **事件总线** | **自研最小版**：类型化 Publish/Subscribe、注册进 IoC、订阅凭据可释放、主线程、异常隔离 | v0.4 新增决策；不做优先级/粘性/异步分发；边界纪律见 Core 设计稿 |
| 命名空间 | `Zipper.*` | 与现状一致 |
...
- **事件总线（最小版）**：类型化 `Publish/Subscribe`、注册进 IoC（Scope 单例）、订阅凭据可释放、主线程发布、异常隔离；**边界纪律**：只用于"跨模块低频通知"，不用于请求-响应、状态查询、高频数据流（后者走 R3 或直接引用）。
...
| 事件总线滥用 | 易演变为全局状态与隐式控制流，调试困难 | 白名单/黑名单纪律写进 Core 设计稿；订阅凭据强制可释放 |
