...
> 状态：v0.3 草案（核心规则：品牌名 Zipper 取首字母 **Z**、内部机制类型不加前缀；v0.3（2026-09-10）随事件总线单实现化删除 `ZR3EventBus` 示例）
...
| 版本 | 日期 | 说明 |
|---|---|---|
| v0.3 | 2026-09-10 | **删除 `ZR3EventBus` 示例**（事件总线由双实现收敛为单实现，见 `core-design.md` §4.3 / v0.7）：§2 保留"技术栈标识优先"规则但不再举该类型为正例、不预先虚构替代名；§4 前缀正例表移除 `ZR3EventBus` |
